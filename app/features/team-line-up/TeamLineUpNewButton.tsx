...
export function TeamLineUpNewButton({
  editorBasePath = "/editor",
  buttonLabel = "New Team Line-Up",
  modalTitle = "Team Line-Up template",
}: {
  ...
  const create = async (data?: Sport365LineupImport) => {
    ...
    setError(null);
    try {
      const res = await fetch("/api/templates/team-line-up/create", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ data: imp, brandStyle }),
      });
    ...
    <div className="space-y-1">
      ...
      {open && (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-labelledby="team-line-up-new-title"
          onClick={() => {
            if (!busy) setOpen(false);
          }}
        >
          <div className="ui-modal relative z-10 w-full max-w-lg p-5" onClick={(ev) => ev.stopPropagation()}>
            <h2 id="team-line-up-new-title" className="text-lg font-semibold text-[color:var(--text-primary)]">
              {modalTitle}
            </h2>
            <p className="mt-2 text-sm text-[color:var(--text-secondary)]">
              Paste a <strong className="font-semibold text-[color:var(--text-primary)]">Sport365 Match Centre</strong>{" "}
              URL to import formations, player positions, and kit colours for tactical pitch graphics.
            </p>
