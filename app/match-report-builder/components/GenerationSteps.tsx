...
function EditableMatchReportOutput({
  project,
  busy,
  onProjectChange,
  run,
  ...
  const repairWithAi = async () => {
    const res = await fetch(studioApiPath(`/api/match-report/projects/${encodeURIComponent(project.id)}/repair-fact-check`), {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    });
    const data = await parseApiJson<{ project?: MatchReportProject; error?: string }>(res);
  ...
  return (
    <div className="space-y-3">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.18em] text-emerald-400">Editable report output</p>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <R365Button type="button" onClick={() => run(repairWithAi)} disabled={busy || !project.factCheck?.issues.length}>
            AI fix issues
          </R365Button>
        </div>
      </div>
    ...
    return (
      <div className="space-y-4">
        ...
        <div>
          ...
          <p className="mt-2 text-sm text-[color:var(--text-secondary)]">
            Generate headline, standfirst, Match Analysis, Extended Report, player ratings, and 16 Conclusions from the EIO.
          </p>
        </div>
  ...
  if (step === "fact_check") {
    return (
      <div className="space-y-4">
        ...
        <EditableMatchReportOutput project={project} busy={busy} onProjectChange={onProjectChange} run={run} />
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <R365Button
            onClick={() =>
              run(async () => {
                const res = await fetch(studioApiPath(`/api/match-report/projects/${encodeURIComponent(project.id)}/fact-check`), {
                  method: "POST",
                  headers: { "Content-Type": "application/json" },
                });
                const data = await parseApiJson<{ project?: MatchReportProject; error?: string }>(res);
            ...
            disabled={busy || !project.mediaOutputs}
          >
