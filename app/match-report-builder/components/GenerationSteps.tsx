...
export function GenerationSteps({
  ...
  const [includeSixteen, setIncludeSixteen] = useState(true);
    ...
    return (
      <div className="space-y-4">
        ...
        <div>
          ...
          <p className="mt-2 text-sm text-[color:var(--text-secondary)]">
            Generate headline, standfirst, full report, player ratings, and optional 16 conclusions from the EIO.
          </p>
        </div>
        <label className="flex items-center gap-2 text-sm text-[color:var(--text-secondary)]">
          <input
            type="checkbox"
            checked={includeSixteen}
            onChange={(e) => setIncludeSixteen(e.target.checked)}
          />
          Include 16 conclusions
        </label>
                  ...
                  try {
                    res = await fetch(studioApiPath("/api/match-report/generate-media"), {
                      ...
                      body: JSON.stringify({
                        ...
                        includeSixteenConclusions: includeSixteen,
                      }),
                    });
                  } catch (e) {
                    const message = e instanceof Error ? e.message : "Network error";
