...
import { workflowArchiveBodySchema } from "@/features/editing-studio/validators/workflow-schemas";
...
export async function POST(req: Request, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!canPerformWorkflowAction(role, project.status, "archive")) {
      return NextResponse.json(
        { error: "Forbidden", reason: workflowActionDisabledReason(role, project.status, "archive") },
        { status: 403 },
      );
    }
    const raw = (await req.json().catch(() => ({}))) as unknown;
    const body = workflowArchiveBodySchema.parse(raw);
    const next = await repo.workflowArchive(id, { role, actor, note: body.note });
