...
export default async function F1GridPage({
  ...
  const items = await getRacingDataProvider().getF1GridBundles();

  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <h1 className="text-3xl font-black text-white">F1 Grid Template</h1>
          <p className="mt-2 max-w-xl text-slate-400">
            ...
            Grid 1 (page 1/2), Grid 2 (page 2/2), Outro. Export PNG frames and Shorts MP4 from the editor. Saved
            templates use{" "}
            <code className="text-slate-500">tpl-…</code> ids in{" "}
            <code className="text-slate-500">data/local/user-templates.json</code>. Demo data:{" "}
            <code className="text-slate-500">data/dummy/f1-grid.json</code>.
          </p>
        </div>
        <CreateTemplateButton format="f1-grid" editorBasePath={editorBasePath} />
      </div>

      <div className="grid gap-4 md:grid-cols-2">
        {items.length === 0 && (
          <p className="text-sm text-slate-500 md:col-span-2">
            No grids yet — click <strong className="text-slate-400">New template</strong> to create a{" "}
          ...
          return (
            <Panel key={b.id} title={title.slice(0, 80) + (title.length > 80 ? "…" : "")}>
              <div className="flex flex-wrap items-center gap-2">
                ...
                <DeleteTemplateButton format="f1-grid" id={b.id} />
              </div>
              ...
              <Link
                href={`${editorBasePath}/f1-grid/${b.id}`}
