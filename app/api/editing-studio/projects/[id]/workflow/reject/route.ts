...
import { workflowRejectBodySchema } from "@/features/editing-studio/validators/workflow-schemas";
...
export async function POST(req: Request, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!canPerformWorkflowAction(role, project.status, "reject")) {
      return NextResponse.json(
        { error: "Forbidden", reason: workflowActionDisabledReason(role, project.status, "reject") },
        { status: 403 },
      );
    }
    const raw = (await req.json().catch(() => ({}))) as unknown;
    const body = workflowRejectBodySchema.parse(raw);
    const next = await repo.workflowReject(id, { role, actor, note: body.note });
