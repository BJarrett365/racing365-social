...
function outputRules(outputType: ScriptOutputType): string {
  if (outputType === "article") {
    return [
      ...
      "Write in a clean football news feature style for Planet Sport Studio.",
    ].join(" ");
  }
...
export async function generateYouTubeScriptOutput(input: GenerateInput): Promise<YouTubeGeneratedOutput> {
  ...
  const prompt = [
    ...
    `Brand tone: ${input.brandTone?.trim() || "Planet Sport Studio editorial, clear and useful"}`,
        ...
        messages: [
          {
            role: "system",
            content:
              "You are Planet Sport Studio YouTube Script Importer. Produce useful editorial outputs from transcripts, obeying compliance and fact-preservation rules.",
          },
